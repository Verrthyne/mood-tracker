...
- `fixtures/README.md`
  - 開発用合成データの目的と再生成方法
- `fixtures/synthetic-backup-2026-04-01-to-2026-07-14.json`
...
- `fatigue`: しんどさ（肉体的な苦しさ）
- `interest`: 関心
- `heaviness`: 気分の重さ（精神的な苦しさ）

入力画面、記録一覧、グラフ、統計、曜日別・時間帯別・タグ別分析でも、この区別が分かる表示名を使用する。
...
- 対象記録の日付・時刻と、操作を元に戻せないことを確認画面に表示する。
- 確認をキャンセルした場合は、記録と保存データを変更しない。
- 確認した場合だけ、対象ID以外の記録を残して `localStorage`へ保存する。
- Undoとごみ箱はない。
...
開発・動作確認および公開版での運用確認用として、2026-04-01から2026-07-14までの合成データを固定シードで生成した。
...
- 雨タグ付き記録: 44件
- 休みタグ付き記録: 60件
- 外出記録: 52件（外食28件、研修17件、通院7件）
- 心身の負担が高くても関心が70以上の記録: 4件
- 雨あり平均: しんどさ60、気分の重さ58
- 雨なし平均: しんどさ39、気分の重さ35
- 休み平均: しんどさ31、気分の重さ27
- 休み以外平均: しんどさ55、気分の重さ50
...
```text
../private-backups/mood-tracker-synthetic-2026-04-01-to-2026-07-14.json
```

両ファイルのSHA-256:

```text
414ef5744034ec1ee361e9fba2a6efb74e02b399899e0154cdb3dfafc318cfa7
```
...
標準指標の意味の明確化について、localhostで次を確認した。

- 入力、統計、記録一覧、グラフ、週次比較、曜日別・時間帯別・タグ別分析に「肉体」「精神」の区別が表示される
- 390×844のグラフ・分析画面で横スクロールが発生しない

記録削除の安全化について、localhostと隔離実行で次を確認した。

- 削除ボタンのアクセシブル名に対象記録の日付・時刻が含まれる
- 確認を承認した場合だけ対象記録を削除し、再読み込み後も削除状態を保持する
- 確認をキャンセルした場合は記録を残し、`localStorage`への書き込みを行わない
...
- IDが155件すべて一意
- 2026-04-01から2026-07-14までの全日を含む
...
- 記録削除前の日時付き確認と、削除ボタンの記録別アクセシブル名
- 「しんどさ」を肉体的な苦しさ、「気分の重さ」を精神的な苦しさとして表示上で明確化
...
4. 単独記録と外出記録が見えるようにグラフ表示を改善する。
5. 「週次サマリー」の名称または集計方法を実態に合わせる。
6. CSVヘッダー、BOM、数式注入対策を改善する。
7. タブ、トグル、canvasのアクセシビリティを改善する。
8. CDN取得失敗時の表示とフォールバックを追加する。
9. 通常の記録保存失敗時のエラー処理を追加する。
10. 削除後のUndoまたはごみ箱を追加する。
