...
現状の形式検証は限定的で、`records` が配列かどうかが主な判定条件。
...
- JSONインポートが確認なしで全データを置き換える。
- JSON形式の検証が限定的。
- 削除に確認、Undo、ごみ箱がない。
- `localStorage`の容量超過や保存失敗を利用者へ通知しない。

### セキュリティ

- カスタム指標名・説明を `innerHTML` へ直接挿入する箇所がある。
- 信頼できないJSONをインポートすると、HTML・スクリプト注入の可能性がある。
...
7. 削除に確認またはUndoを追加する。
8. 単独記録と外出記録が見えるようにグラフ表示を改善する。
9. 「週次サマリー」の名称または集計方法を実態に合わせる。
10. CSVヘッダー、BOM、数式注入対策を改善する。
11. タブ、トグル、削除ボタン、canvasのアクセシビリティを改善する。
12. CDN取得失敗時の表示とフォールバックを追加する。
13. 保存失敗時のエラー処理を追加する。
14. 自動テストまたは最低限の回帰確認手順を整備する。
15. `.nojekyll` と空の `download` の扱いを整理する。
